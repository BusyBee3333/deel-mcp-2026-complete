...
import type { ApiResponse, Document, DocumentTemplate } from "../types.js";
...
export function registerDocumentTools(server: McpServer): void {
  ...
  server.tool(
    ...
    async ({ contract_id, worker_id, status, type, limit, offset }) => {
      ...
      const result = await client.get<ApiResponse<Document[]>>(
        "/v2/documents",
        params
      );
        ...
        content: [
          {
            type: "text",
            text: JSON.stringify(
              { documents, meta: result.meta, count: documents.length },
              null,
              2
            ),
          },
  ...
  server.tool(
    ...
    async ({ document_id, signature }) => {
      const client = getClient();
      const result = await client.post<ApiResponse<Document>>(
        `/v2/documents/${document_id}/sign`,
        { signature }
      );
      return {
        content: [{ type: "text", text: JSON.stringify(result.data, null, 2) }],
  ...
  server.tool(
    ...
    async ({ type, country, limit, offset }) => {
      ...
      const result = await client.get<ApiResponse<DocumentTemplate[]>>(
        ...
        content: [
          {
            type: "text",
            text: JSON.stringify(
              { templates, meta: result.meta, count: templates.length },
              null,
              2
            ),
          },
  ...
  server.tool(
    ...
    async ({ template_id }) => {
      const client = getClient();
      const result = await client.get<ApiResponse<DocumentTemplate>>(
        `/v2/document-templates/${template_id}`
      );
      return {
        content: [{ type: "text", text: JSON.stringify(result.data, null, 2) }],
