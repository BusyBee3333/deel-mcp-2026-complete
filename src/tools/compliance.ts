...
export function registerComplianceTools(server: McpServer): void {
  ...
  server.tool(
    ...
    async ({ worker_id, contract_id, country, status, type, limit, offset }) => {
      ...
      const result = await client.get<ApiResponse<ComplianceItem[]>>(
        "/v2/compliance",
        params
      );
        ...
        content: [
          {
            type: "text",
            text: JSON.stringify(
              { compliance_items: items, meta: result.meta, count: items.length },
              null,
              2
            ),
          },
  ...
  server.tool(
    ...
    async ({ worker_id, contract_id, country }) => {
      ...
      const result = await client.get<ApiResponse<ComplianceStatus>>(
  ...
  server.tool(
    ...
    async ({ worker_id, contract_id, country, is_submitted, limit, offset }) => {
      ...
      const result = await client.get<ApiResponse<RequiredDocument[]>>(
        "/v2/compliance/required-documents",
        params
      );
        ...
        content: [
          {
            type: "text",
            text: JSON.stringify(
              { required_documents: documents, meta: result.meta, count: documents.length },
              null,
              2
            ),
          },
