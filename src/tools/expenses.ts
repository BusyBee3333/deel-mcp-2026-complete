...
import type { ApiResponse, Expense } from "../types.js";
...
export function registerExpenseTools(server: McpServer): void {
  ...
  server.tool(
    ...
    async ({ contract_id, status, category, from_date, to_date, limit, offset }) => {
      ...
      const result = await client.get<ApiResponse<Expense[]>>(
        "/v2/expenses",
        params
      );
        ...
        content: [
          {
            type: "text",
            text: JSON.stringify(
              { expenses, meta: result.meta, count: expenses.length },
              null,
              2
            ),
          },
  ...
  server.tool(
    ...
    async (payload) => {
      const client = getClient();
      const result = await client.post<ApiResponse<Expense>>(
        "/v2/expenses",
        payload
      );
      return {
  ...
  server.tool(
    ...
    async ({ expense_id, notes }) => {
      const client = getClient();
      const result = await client.post<ApiResponse<Expense>>(
        `/v2/expenses/${expense_id}/approve`,
        { notes }
      );
      return {
        content: [{ type: "text", text: JSON.stringify(result.data, null, 2) }],
  ...
  server.tool(
    ...
    async ({ expense_id, reason }) => {
      const client = getClient();
      const result = await client.post<ApiResponse<Expense>>(
        `/v2/expenses/${expense_id}/decline`,
        { reason }
      );
      return {
        content: [{ type: "text", text: JSON.stringify(result.data, null, 2) }],
