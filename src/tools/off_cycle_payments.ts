...
import type { ApiResponse, OffCyclePayment } from "../types.js";

export function registerOffCyclePaymentTools(server: McpServer): void {
  ...
  server.tool(
    ...
    async ({ contract_id, status, from_date, to_date, limit, offset }) => {
      ...
      const result = await client.get<ApiResponse<OffCyclePayment[]>>(
        "/v2/off-cycle-payments",
        params
      );
        ...
        content: [
          {
            type: "text",
            text: JSON.stringify(
              { off_cycle_payments: payments, meta: result.meta, count: payments.length },
              null,
              2
            ),
          },
  ...
  server.tool(
    ...
    async (payload) => {
      const client = getClient();
      const result = await client.post<ApiResponse<OffCyclePayment>>(
        "/v2/off-cycle-payments",
        payload
      );
      return {
  ...
  server.tool(
    ...
    async ({ payment_id }) => {
      const client = getClient();
      const result = await client.get<ApiResponse<OffCyclePayment>>(
        `/v2/off-cycle-payments/${payment_id}`
      );
      return {
        content: [{ type: "text", text: JSON.stringify(result.data, null, 2) }],
  ...
  server.tool(
    ...
    async ({ payment_id, notes }) => {
      const client = getClient();
      const result = await client.post<ApiResponse<OffCyclePayment>>(
        `/v2/off-cycle-payments/${payment_id}/approve`,
        { notes }
      );
      return {
        content: [{ type: "text", text: JSON.stringify(result.data, null, 2) }],
