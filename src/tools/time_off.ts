...
import type { ApiResponse, TimeOffPolicy, TimeOffRequest } from "../types.js";
...
export function registerTimeOffTools(server: McpServer): void {
  ...
  server.tool(
    ...
    async ({ country, type, limit, offset }) => {
      ...
      const result = await client.get<ApiResponse<TimeOffPolicy[]>>(
        "/v2/time-off/policies",
        params
      );
        ...
        content: [
          {
            type: "text",
            text: JSON.stringify(
              { policies, meta: result.meta, count: policies.length },
              null,
              2
            ),
          },
  ...
  server.tool(
    ...
    async ({ worker_id, status, from_date, to_date, limit, offset }) => {
      ...
      const result = await client.get<ApiResponse<TimeOffRequest[]>>(
        "/v2/time-off/requests",
        params
      );
        ...
        content: [
          {
            type: "text",
            text: JSON.stringify(
              { requests, meta: result.meta, count: requests.length },
              null,
              2
            ),
          },
  ...
  server.tool(
    ...
    async (payload) => {
      const client = getClient();
      const result = await client.post<ApiResponse<TimeOffRequest>>(
        "/v2/time-off/requests",
        payload
      );
      return {
  ...
  server.tool(
    ...
    async ({ request_id, notes }) => {
      const client = getClient();
      const result = await client.post<ApiResponse<TimeOffRequest>>(
        `/v2/time-off/requests/${request_id}/approve`,
        { notes }
      );
      return {
        content: [{ type: "text", text: JSON.stringify(result.data, null, 2) }],
