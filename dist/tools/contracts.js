...
export function registerContractTools(server) {
    // ── list_contracts ───────────────────────────────────────────────────────
    server.tool("list_contracts", "List all contracts in your Deel organization. Supports filtering by status, type, and worker.", {
        status: z
        ...
        const result = await client.get("/v2/contracts", params);
    ...
    // ── get_contract ─────────────────────────────────────────────────────────
    server.tool("get_contract", "Get a single contract by ID with full details.", {
        ...
        const client = getClient();
        const result = await client.get(`/v2/contracts/${contract_id}`);
        return {
            content: [
    ...
    // ── create_contract ──────────────────────────────────────────────────────
    server.tool("create_contract", "Create a new contract in Deel.", {
        type: z
            .enum(["eor", "contractor", "employee", "pay_as_you_go", "fixed"])
        ...
        const client = getClient();
        const result = await client.post("/v2/contracts", payload);
        return {
            content: [
                { type: "text", text: JSON.stringify(result.data, null, 2) },
            ],
    ...
    // ── update_contract ──────────────────────────────────────────────────────
    server.tool("update_contract", "Update an existing contract's details.", {
        ...
        const client = getClient();
        const result = await client.patch(`/v2/contracts/${contract_id}`, updates);
        return {
            content: [
                { type: "text", text: JSON.stringify(result.data, null, 2) },
            ],
    ...
    // ── terminate_contract ───────────────────────────────────────────────────
    server.tool("terminate_contract", "Terminate an active contract.", {
        ...
        const client = getClient();
        const result = await client.post(`/v2/contracts/${contract_id}/terminate`, payload);
        return {
            content: [
                { type: "text", text: JSON.stringify(result.data, null, 2) },
            ],
    ...
    server.tool("get_contract_documents", "Retrieve all documents associated with a contract.", {
        contract_id: z.string().describe("The contract ID"),
        ...
        const client = getClient();
        const result = await client.get(`/v2/contracts/${contract_id}/documents`, { limit, offset });
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify({ documents: result.data, meta: result.meta }, null, 2),
                },
            ],
            structuredContent: {
