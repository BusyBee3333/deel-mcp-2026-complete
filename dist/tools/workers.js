...
export function registerWorkerTools(server) {
    // ── list_workers ─────────────────────────────────────────────────────────
    server.tool("list_workers", "List all workers in your Deel organization.", {
        status: z
        ...
        const result = await client.get("/v2/workers", params);
        const workers = result.data ?? [];
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify({ workers, meta: result.meta, count: workers.length }, null, 2),
                },
            ],
    ...
    // ── get_worker ───────────────────────────────────────────────────────────
    server.tool("get_worker", "Get detailed information about a specific worker.", {
        ...
        const client = getClient();
        const result = await client.get(`/v2/workers/${worker_id}`);
        return {
    ...
    // ── invite_worker ────────────────────────────────────────────────────────
    server.tool("invite_worker", "Invite a new worker to join Deel.", {
        ...
        const client = getClient();
        const result = await client.post("/v2/workers/invite", payload);
        return {
            content: [{ type: "text", text: JSON.stringify(result.data, null, 2) }],
    ...
    // ── update_worker ────────────────────────────────────────────────────────
    server.tool("update_worker", "Update an existing worker's profile information.", {
        ...
        const client = getClient();
        const result = await client.patch(`/v2/workers/${worker_id}`, updates);
        return {
            content: [{ type: "text", text: JSON.stringify(result.data, null, 2) }],
    ...
    // ── offboard_worker ──────────────────────────────────────────────────────
    server.tool("offboard_worker", "Initiate the offboarding process for a worker.", {
        ...
        const client = getClient();
        const result = await client.post(`/v2/workers/${worker_id}/offboard`, payload);
        return {
            content: [{ type: "text", text: JSON.stringify(result.data, null, 2) }],
    ...
    // ── search_workers ───────────────────────────────────────────────────────
    server.tool("search_workers", "Search for workers by name or email.", {
        query: z
            .string()
        ...
        const client = getClient();
        const result = await client.get("/v2/workers", {
            q: query,
            limit,
            offset,
        });
        ...
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify({ workers, meta: result.meta, count: workers.length, query }, null, 2),
                },
            ],
            structuredContent: {
                workers,
