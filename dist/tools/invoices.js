...
export function registerInvoiceTools(server) {
    // ── list_invoices ────────────────────────────────────────────────────────
    server.tool("list_invoices", "List invoices with optional filters by status, contract, or date range.", {
        ...
        const result = await client.get("/v2/invoices", params);
        const invoices = result.data ?? [];
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify({ invoices, meta: result.meta, count: invoices.length }, null, 2),
                },
            ],
    ...
    // ── get_invoice ──────────────────────────────────────────────────────────
    server.tool("get_invoice", "Get a specific invoice by ID with full line item details.", {
        ...
        const client = getClient();
        const result = await client.get(`/v2/invoices/${invoice_id}`);
        return {
    ...
    // ── approve_invoice ──────────────────────────────────────────────────────
    server.tool("approve_invoice", "Approve a pending invoice for payment.", {
        ...
        const client = getClient();
        const result = await client.post(`/v2/invoices/${invoice_id}/approve`, { notes });
        return {
            content: [{ type: "text", text: JSON.stringify(result.data, null, 2) }],
    ...
    // ── decline_invoice ──────────────────────────────────────────────────────
    server.tool("decline_invoice", "Decline a pending invoice with a reason.", {
        ...
        const client = getClient();
        const result = await client.post(`/v2/invoices/${invoice_id}/decline`, { reason });
        return {
            content: [{ type: "text", text: JSON.stringify(result.data, null, 2) }],
    ...
    server.tool("list_invoice_adjustments", "List adjustments (bonuses, deductions) for an invoice.", {
        invoice_id: z.string().describe("The invoice ID"),
        limit: z.number().int().min(1).max(100).default(20).describe("Results per page"),
        offset: z.number().int().min(0).default(0).describe("Pagination offset"),
    }, {
        ...
        const client = getClient();
        const result = await client.get(`/v2/invoices/${invoice_id}/adjustments`, { limit, offset });
        const adjustments = result.data ?? [];
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify({ adjustments, meta: result.meta, invoice_id }, null, 2),
                },
            ],
